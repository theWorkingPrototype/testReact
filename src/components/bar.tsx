...
<svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px"
	style={{padding: '5px', marginRight: '5px'}} width="40px" height="40px" viewBox="0 0 209.322 209.322">
<g>
...
function Bar (
  ...
  const { logo, tabs, bold } = props;
  const [active, setActive] = useState(0);
  let logoElement:ReactElement = <span></span>;
  if(logo) logoElement = logoJSX;
  const optJSX = tabs.map((tab, index) => {
    let cn = 'tab';
    if(index == active) cn += ' active';
    if(bold) cn += ' bold';
    return (
      <div className={cn} key={'tab-' + index} onClick={() => setActive(index)}>
        {tab}
      </div>
    );
  });
